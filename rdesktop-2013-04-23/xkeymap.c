...
/*
 *
			static unsigned int auto_flag = 0;
			if( auto_flag < 50 && pressed ){
				auto_flag++;
				printf("Toggle auto actions\n");//zhongbin
			//	ui_move_pointer(950 - (auto_flag % 4) * 60 , 600);
			//	rdp_send_input(time(NULL), RDP_INPUT_MOUSE, MOUSE_FLAG_MOVE, 950 - (auto_flag % 4) * 60 , 600);
			//	rdp_send_input(time(NULL), RDP_INPUT_MOUSE, 0xa000, 950 - (auto_flag % 4) * 60 , 600);
			//	rdp_send_input(time(NULL), RDP_INPUT_MOUSE, 0x2000, 950 - (auto_flag % 4) * 60 , 600);
				if(auto_flag == 1){
					Toggled = True;	
				}
			}
			else if( auto_flag <100 && pressed ){
				printf("trying to fake a key press\n");
				XTestFakeButtonEvent(g_display, 0xff0d, True, 0);
				XTestFakeButtonEvent(g_display, 0xff0d, False, 0);
			}
*/
			break;
...
{
//	printf("key press or release\n%hx	%s\n", keysym, get_ksname(keysym));//zhongbin
...
//		printf("%u	%hx	%hx\n",ev_time, pressed ? RDP_KEYPRESS : RDP_KEYRELEASE, tr.scancode);//zhongbin
/*
 * 		if(RECORD){
...
*/
	...
	if (pressed)
	{
		...
		//	printf("-seq-key pressed\n%hx	%hx	%hx	%u	%u\n",
		//		ptr->seq_keysym, keycode, state, ev_time,  nesting);//zhongbin
			xkeymap_send_keys(ptr->seq_keysym, keycode, state, ev_time, False, nesting);
		//	printf("-seq-key released\n%hx	%hx	%hx	%u	%u\n",
		//		ptr->seq_keysym, keycode, state, ev_time,  nesting);//zhongbin
			ptr = ptr->next;
		}
		while (ptr);
...
//		printf("rdp_input_scancode:%hx\nscancode:%hx time:%u\n\n",
//			RDP_INPUT_SCANCODE, scancode, time);//zhongbin
...
//		printf("rdp_input_scancode:%hx\nscancode:%hx time:%u flags:%hx\n\n",
//			RDP_INPUT_SCANCODE, scancode, time, flags);//zhongbin
	}
}
