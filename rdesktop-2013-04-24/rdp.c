...
//			printf("rdpver == 0xff\n");//zhongbin
		...
		{
		//	printf("rdp5_process was called type=0\n");//zhongbin
...
//	printf("normal return from rdp_recv\n");//zhongbin
	return rdp_s;
}
...
rdp_loop(RD_BOOL * deactivated, uint32 * ext_disc_reason)
{
	...
	RD_BOOL cont = True;
	STREAM s;
//printf("cont:%d time:%u\n", cont, time(NULL));//zhongbin
	while (cont)
	{
		s = rdp_recv(&type);
		if (s == NULL){
		...
		switch (type)
		{
			...
			//	printf("Get a RDP_PDU_DEMAND_ACTIVE type stream at %d\n", time((time_t *)NULL));//zhongbin
				process_demand_active(s);
				*deactivated = False;
				break;
			case RDP_PDU_DEACTIVATE:
			//	printf("get a RDP_PDU_DEACTIVATE type stream at %d\n", time((time_t *)NULL));//zhongbin
				DEBUG(("RDP_PDU_DEACTIVATE\n"));
				*deactivated = True;
				break;
			case RDP_PDU_REDIRECT:
			//	printf("get a RDP_PDU_REDIRECT type stream at %d\n", time((time_t *)NULL));//zhongbin
				return process_redirect_pdu(s);
				break;
			case RDP_PDU_DATA:
			//	printf("get a RDP_PDU_DATA type stream at %d\n", time((time_t *)NULL));//zhongbin
				process_data_pdu(s, ext_disc_reason);
				break;
			case 0:
				/*after the connection is established, almost all the stream type in rdp-level is this type*/
			//	printf("get a 0 type stream at %d\n", time((time_t *)NULL));//zhongbin
				break;
			default:
			//	printf("get a unknown type stream at %d\n", time((time_t *)NULL));//zhongbin
				unimpl("PDU %d\n", type);
		}
