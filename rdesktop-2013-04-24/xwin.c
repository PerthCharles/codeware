...
//add for record  --zhongbin
#include <signal.h>			/*The head file is add for alarm() by zhongbin*/

/*order type processed by process_orders() orders.c line 1309*/
extern uint16 order_count;
extern uint16 destblt_count;
extern uint16 patblt_count;
extern uint16 screenblt_count;
extern uint16 line_count;
extern uint16 rect_count;
extern uint16 desksave_count;
extern uint16 memblt_count;
extern uint16 triblt_count;
extern uint16 polygon_count;
extern uint16 polygon2_count;
extern uint16 polyline_count;
extern uint16 ellipse_count;
extern uint16 ellipse2_count;
extern uint16 text2_count;
//RD_BOOL CLOSE = False;
/*Define the itimerval for setitimer()*/
struct itimerval timer;

#define ACTION_TIMEOUT 1		/*Each action is played with 5 sceonds wait*/
unsigned int action_count = 0;		/*Counts for actions which have been sent*/
RD_BOOL action_can_send = True;		/*whether the next action can be send or not*/
//unsigned int waiting_action = 1;	/*The action number which will be send when invoked by alarm*/

RD_BOOL record_send = False;		/*flag of whether the package is send by record file*/
extern FILE *record_file;		/*The file you are going to read or write*/
extern RD_BOOL RECORD;			/*If is True, program will record your operation */
extern RD_BOOL PLAY_RECORD;
extern RD_BOOL NOT_END;			/*If is True, program will continue to play the record file until the end*/
extern RD_BOOL Toggled;			/*If is True, program will start to play the record file*/
//The following statements is temporary used for play the record file
int re_type;	/* type of each record entry.
			0 means need call rdp_send_input
			1 means need call handle_special_keys
			2 means need call xkeymap_send_keys
			15 means this is the end of record file
		*/
//parameter of rdp_send_input()
uint32 re_time;
uint16 re_message_type;
uint16 re_device_flags;
uint16 re_param1, re_param2;
//parameter of handle_special_keys() and xkeymap_send_keys()
uint32 re_keysym;
unsigned int re_keycode;
unsigned int re_state;
uint32 re_ev_time;
RD_BOOL re_pressed;
uint8 re_nesting;
...
//		printf("xevent.type==ButtonRelease\nbutton:%hd\n\n",button);//zhongbin
//		printf("rdp_input_mouse:%x\n\n",RDP_INPUT_MOUSE);
	...
	//	printf("Button normal press\n%u	%hx	%hx	%d	%d\n",
	//		time(NULL), RDP_INPUT_MOUSE, flags|button, xevent.xbutton.x, xevent.xbutton.y);//zhongbin
		if(RECORD){
		//	if( fprintf(record_file, "%u\n%u %hx %hx %d %d\n", 0, time(NULL), RDP_INPUT_MOUSE, MOUSE_FLAG_MOVE, xevent.xmotion.x, xevent.xmotion.y) == 0){
		//		printf("Error:Something wrong with writing record file\n");
		//	}
			if( fprintf(record_file, "%u\n%u %hx %hx %d %d\n", 0, time(NULL), RDP_INPUT_MOUSE, flags | button, xevent.xbutton.x, xevent.xbutton.y) == 0){
			printf("Error:Something wrong with writing record file line 2323 in xwin.c\n");
			}
		}
	//	printf("flags:%x  button:%x \n", flags, button);
	}
	else
	{
		/* SeamlessRDP */
...
/*when next record entry want send, the following values must be clear --zhongbin*/
void clear_orders_count()
{
	order_count = 0;
	destblt_count = 0;
	patblt_count = 0;
...
//zhongbin
void printf_orders_count()
{
	//printf("%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t\n",
		//order_count, destblt_count, patblt_count, screenblt_count, line_count, rect_count, desksave_count, memblt_count, triblt_count, 
		//polygon_count, polygon2_count, polyline_count, ellipse_count, ellipse2_count, text2_count);
	printf("%d\t%d\t%d\t%d\t%d\t%d\t\n",
		order_count, patblt_count, rect_count, memblt_count, polygon_count, text2_count);
}

/*play the record file -- zhongbin*/
void play_record(int signal)
{
//	if(PLAY_RECORD){
//		printf("PLAY_RECORD:%d	NOT_END:%d\n", PLAY_RECORD, NOT_END);
//	}
	if(PLAY_RECORD && NOT_END){
		if( fscanf(record_file, "%d", &re_type) != -1){
			switch(re_type)
			{
				case 0:
					record_send = True;//zhongbin
					fscanf(record_file, "%u %hx %hx %d %d", &re_time, &re_message_type, &re_device_flags, &re_param1, &re_param2);
				//	printf("%u %hx %hx %d %d\n", re_time, re_message_type, re_device_flags, re_param1, re_param2);
				//	rdp_send_input(re_time, re_message_type, re_device_flags, re_param1, re_param2);
					printf_orders_count();
					if(re_device_flags == MOUSE_FLAG_MOVE){
						ui_move_pointer(re_param1,re_param2);
					}
					else{	
						//printf("[record mouse clik]		");//zhongbin
						rdp_send_input(time( (time_t *)NULL), re_message_type, re_device_flags, re_param1, re_param2);
					}
					action_count++;
					action_can_send = True;
					if(re_device_flags != MOUSE_FLAG_MOVE){
					...
					record_send = False;//zhongbin	
					clear_orders_count();
					break;
				case 1:
				//	printf("[record special keys]\n");//zhongbin
					record_send = True;//zhongbin
					fscanf(record_file, "%lx %x %u %u", &re_keysym, &re_state, &re_ev_time, &re_pressed);
				//	printf_orders_count();
				//	printf("%lx %x %u %u\n", re_keysym, re_state, re_ev_time, re_pressed);
				//	handle_special_keys(re_keysym, re_state, re_ev_time, re_pressed);
					handle_special_keys(re_keysym, re_state, time( (time_t *)NULL), re_pressed);
					action_count++;
					action_can_send = True;
					timer.it_value.tv_sec = 4;
					//timer.it_value.tv_sec = 0;
				...
				//	clear_orders_count();
					break;
				case 2:
				//	printf("[record sent keys]		");//zhongbin
					record_send = True;//zhongbin
					fscanf(record_file, "%lx %x %x %u %u %u", &re_keysym, &re_keycode, &re_state, &re_ev_time, &re_pressed, &re_nesting);
					printf_orders_count();
				//	printf("%lx %x %x %u %u %u\n", re_keysym, re_keycode, re_state, re_ev_time, re_pressed, re_nesting);
				//	xkeymap_send_keys(re_keysym, re_keycode, re_state, re_ev_time, re_pressed, re_nesting);
					xkeymap_send_keys(re_keysym, re_keycode, re_state, time( (time_t *)NULL), re_pressed, re_nesting);
					action_count++;
					action_can_send = True;
					//if the pressed key is return,set the timer longer
					if(re_keysym == 0xff0d){
						timer.it_value.tv_sec = 4;
					}
					else{
				...
				case 15:
					printf_orders_count();
					NOT_END = False;
				//	printf("Here is the end of record file\n");
					fclose(record_file);
					//CLOSE = True;
					clear_orders_count();
					break;
...
/*handle the timing of send action --zhongbin*/
void set_play_signal()
{
	timer.it_value.tv_usec = 100000;	//100ms
	timer.it_interval.tv_sec = 0;
	timer.it_interval.tv_usec = 0;
//	printf("timer.it_value.tv_sec:%d\n",timer.it_value.tv_sec);
//	printf("action_can_send:%u	Toggled:%u\n",action_can_send, Toggled);
	if(action_can_send && Toggled){
		action_can_send = False;
	//	printf("\naction_count:%u\n", action_count);
		signal(SIGALRM, play_record);
	//	alarm(ACTION_TIMEOUT);
	...
	while ((XPending(g_display) > 0) && events++ < 20)
	{
		...
		switch (xevent.type)
		{
			...
			case KeyPress:
				...
				ev_time = time(NULL);
//				printf("special pressed\n%hx	%hx	%u\n", keysym, xevent.xkey.state, ev_time);//zhongbin
				if(RECORD){
					if( fprintf(record_file, "%u\n%lx %x %u %u\n", 1, keysym, xevent.xkey.state, ev_time, True) == 0){
						printf("Error:Something wrong with writing record file\n");
					}
				}
				...
				if (handle_special_keys(keysym, xevent.xkey.state, ev_time, True))
					break;
//				printf("normal pressed\n%hx	%xh	%hx	%u\n", keysym, xevent.xkey.keycode, xevent.xkey.state, ev_time);//zhongbin
				if(RECORD){
					if( fprintf(record_file, "%u\n%lx %x %x %u %u %u\n", 2, keysym, xevent.xkey.keycode, xevent.xkey.state, ev_time, True, 0) == 0){
						printf("Error:Something wrong with writing record file\n");
					}
				}
			...
			case KeyRelease:
				...
				ev_time = time(NULL);
//				printf("special release\n%hx	%hx	%u\n", keysym, xevent.xkey.state, ev_time);//zhongbin
				if(RECORD){
					if( fprintf(record_file, "%u\n%lx %x %u %u\n", 1, keysym, xevent.xkey.state, ev_time, False) == 0){
						printf("Error:Something wrong with writing record file\n");
					}
				}
...
//				printf("normal release\n%hx	%xh	%hx	%u\n", keysym, xevent.xkey.keycode, xevent.xkey.state, ev_time);//zhongbin
				if(RECORD){
					if( fprintf(record_file, "%u\n%lx %x %x %u %u %u\n", 2, keysym, xevent.xkey.keycode, xevent.xkey.state, ev_time, False, 0) == 0){
						printf("Error:Something wrong with writing record file\n");
					}
				}
				xkeymap_send_keys(keysym, xevent.xkey.keycode, xevent.xkey.state,
...
//						time(NULL), RDP_INPUT_MOUSE, MOUSE_FLAG_MOVE, xevent.xbutton.x, xevent.xbutton.y);//zhongbin
/*					if(RECORD){
						if( fprintf(record_file, "%u\n%u %hx %hx %d %d\n", 0, time(NULL), RDP_INPUT_MOUSE, MOUSE_FLAG_MOVE, xevent.xmotion.x, xevent.xmotion.y) == 0){
							printf("Error:Something wrong with writing record file\n");
						}
...
*/
	...
	/* set the alarm to replay the operation sequences --zhongbin*/
	if(PLAY_RECORD){
		set_play_signal();
	}
	if(!NOT_END){
...
//		printf("tv.tv_sec:%d	tv.tv_usec:%d\n", tv.tv_sec, tv.tv_usec);//zhongbin
//		printf("n:%d\n", n);
		n++;
		//call select() to know whether the selected file have changed  --zhongbin
		int select_result;
		//switch (select(n, &rfds, &wfds, NULL, &tv))
		select_result = select(n, &rfds, &wfds, NULL, &tv);
		if(select_result != 0 && select_result != -1){
		//	printf("select_result:%d\n",select_result);//zhongbin
		}
		switch (select_result)
		{
			case -1:
				//ignore the EINTR signal --zhongbin
				if(errno != EINTR){		
					error("select: %s\n", strerror(errno));
				}
			//	printf("select error\n");//zhongbin
			...
			//	if (FD_ISSET(rdp_socket, &rfds)) printf("kidding me\n");//zhongbin
				continue;
		}
