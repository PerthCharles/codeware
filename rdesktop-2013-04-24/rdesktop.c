...
char record_file_path[256];	/* The record file you will read or write */
/*zhongbin testing*/
RD_BOOL RECORD = False;  		/*If is True, program will record your operation */
RD_BOOL PLAY_RECORD = False;
RD_BOOL NOT_END = True;                 /*If is True, program will continue to play the record file until the end*/
RD_BOOL Toggled = False;       	        /*If is True, program will start to play the record file*/

FILE *record_file;			/*The file you are going to read or write*/
...
//			   VNCOPT "Au:L:d:s:c:p:n:k:g:fbBeEmzCDKS:T:NX:a:x:Pr:045h?")) != -1)
	while ((c = getopt(argc, argv,
			   VNCOPT "R:t:Au:L:d:s:c:p:n:k:g:fbBeEmzCDKS:T:NX:a:x:Pr:045h?")) != -1)
	{
		switch (c)
		{
			case 'R':
				STRNCPY(record_file_path, optarg, sizeof(record_file_path));
				if( ( record_file = fopen(record_file_path, "w+") ) == NULL ){
					printf("Cannot open a file to write!\n");
					break;
				}
				RECORD = True;
				PLAY_RECORD = False;
				printf("\n*******************\nYou are going to record your operation sequences!\nRecording...\n");
				printf("your record file will store in :%s\n***********************\n",record_file_path);
				break;
			case 't':
				if( RECORD ) break; //you can't record and play it at the same time.
				STRNCPY(record_file_path, optarg, sizeof(record_file_path));
				if( ( record_file = fopen(record_file_path, "r") ) == NULL ){
					printf("The record file must exited!\n");
					break;
				}
				else{
					PLAY_RECORD = True;
				}
				printf("\n**********************\nYou are going to play the recorded operation sequences!\nPlaying the record...\n");
				printf("your record file to be read is in :%s\n**********************\n",record_file_path);
				break;
#ifdef RDP2VNC
			case 'V':
	...
	while (1)
	{
		...
		DEBUG(("Disconnecting...\n"));
		rdp_disconnect();
		//write the end mark(15) into record file  zhongbin
		if(RECORD){		
			fprintf(record_file, "%d\n", 15);
			if( fclose(record_file) !=0 )
				printf("Close record file error\n");
			else
				printf("You hava succeed record your action sequence\n");
		}
		if (g_redirect)
			continue;
