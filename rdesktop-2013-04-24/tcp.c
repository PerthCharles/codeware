...
tcp_send(STREAM s)
{
	...
	if(record_send && (re_type == 1)) printf("[special key]");//zhongbin
	if(record_send) printf("[send by record]	");//zhongbin
	if(Toggled && NOT_END){
		printf("		sent:%d\n",length);//zhongbin
	}
	int sent, total = 0;
	...
	while (total < length)
	{
		...
		//printf("		this time have sent %d byte\n\n",sent);//zhongbin
		if (sent <= 0)
		{
...
tcp_recv(STREAM s, uint32 length)
{
	...
	while (length > 0)
	{
		...
		//printf("######End   to recv\n");//zhongbin
		//printf("	function recv have recived %d byte data\n\n",rcvd);//zhongbin
		if (rcvd < 0)
		{
...
//		static unsigned int pack_count = 0;
//		printf("\na rdp package have recived!\n count:%d\n\n",pack_count++);//zhongbin
		s->end += rcvd;
		length -= rcvd;
	}
